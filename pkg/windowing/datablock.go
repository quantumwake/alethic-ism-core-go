...
// PartsBySource maps source route IDs to their list of BlockParts within a Block.
type PartsBySource map[string][]*BlockPart

// Block groups events that share the same correlation key, partitioned by source.
// Its evictionTime follows a sliding-window pattern — it resets on every new event.
type Block struct {
	key           string
