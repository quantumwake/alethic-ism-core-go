...
// CacheBlockStore manages multiple BlockStores keyed by route ID.
// It runs a background cleanup loop that removes idle stores after storeIdleTTL.
...
// NewCacheBlockStore creates a CacheBlockStore with default settings:
// 2-hour idle TTL and 5-minute cleanup interval.
...
// Get returns the BlockStore for the given route ID, or nil if not found.
...
// GetOrSet returns an existing BlockStore for the route ID, or lazily creates one
// using fn if it doesn't exist yet.
...
// Exists returns true if a BlockStore is cached for the given route ID.
